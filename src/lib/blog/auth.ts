...
async function createDefaultSettings(): Promise<BlogSettings> {
...
export async function updateCredentials(username: string, password: string): Promise<void> {
  ...
  await supabase.from('admin_settings').update(updates).eq('id', 1);
}
...
export async function getAuthSecret(): Promise<string> {
  try {
    ...
    if (error || !data?.auth_secret) {
      ...
      await supabase.from('admin_settings').update({ auth_secret: secret }).eq('id', 1);
      return secret;
    }
